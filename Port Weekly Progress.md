...
- **Known state at FP7 (since resolved):** when the gate flipped a session to complete, the right panel showed `public/partials/build_plan.php` as a **placeholder** — the 5 section headers with filler text and a disabled copy button, because the Compiler Agent hadn't been built yet. **Resolved at FP9:** `ManifestGenerator.php` now populates `$session['build_plan']`, and `build_plan.php` renders the real prompts with copy + download buttons. This note applied only through FP7/FP8.
